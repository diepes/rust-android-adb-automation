...
fn extract_region_and_label_from_filename(
    source_path: &str,
...
fn find_matches(
    ...
    if let (Some(ex), Some(ey)) = (expected_x, expected_y) {
        ...
        let y_max =
            (ey + patch_height + search_margin).min(image_height.saturating_sub(patch_height));
...
fn main() {
    ...
    println!(
        "⏱️  Matching time:         {:.2}ms",
        match_duration.as_secs_f64() * 1000.0
    );
    println!(
        "⏱️  Total time:            {:.2}ms ({:.3}s)",
        total_duration.as_secs_f64() * 1000.0,
        total_duration.as_secs_f64()
    );
    if stats.total_comparisons > 0 {
        println!(
            "⏱️  Avg time per comparison: {:.2}ms",
...
mod tests {
    ...
    fn test_extract_region_and_label_from_patch_filename() {
        ...
        let region =
            extract_region_and_label_from_filename("patch-claim_button-[10,20,100,100].png");
        assert_eq!(
            region,
            Some((Some("claim_button".to_string()), 10, 20, 100, 100))
        );

        // Test with full path
        let region = extract_region_and_label_from_filename(
            "assets/test_images/patch-dialog-[10,20,100,100].png",
        );
        assert_eq!(region, Some((Some("dialog".to_string()), 10, 20, 100, 100)));

        // Test with spaces in coordinates
        let region =
            extract_region_and_label_from_filename("patch-label-[ 550 , 1345 , 500 , 200 ].png");
        assert_eq!(
            region,
            Some((Some("label".to_string()), 550, 1345, 500, 200))
        );
    }
    ...
    fn test_correlation_perfect_match() {
        ...
        let img1 = image::RgbImage::from_fn(10, 10, |_, _| image::Rgb([100u8, 150u8, 200u8]));
        let img2 = img1.clone();

        let correlation = calculate_correlation(&img1, &img2, 0.5);
        assert!(
            (correlation - 1.0).abs() < 0.01,
            "Perfect match should have ~1.0 correlation"
        );
    }
    ...
    fn test_correlation_different_images() {
        ...
        let img1 = image::RgbImage::from_fn(10, 10, |_, _| image::Rgb([255u8, 255u8, 255u8]));
        let img2 = image::RgbImage::from_fn(10, 10, |_, _| image::Rgb([0u8, 0u8, 0u8]));

        let correlation = calculate_correlation(&img1, &img2, 0.5);
        assert!(
            correlation < 0.1,
            "Completely different images should have low correlation"
        );
    }
    ...
    fn test_correlation_size_mismatch() {
        let img1 = image::RgbImage::from_fn(10, 10, |_, _| image::Rgb([100u8, 150u8, 200u8]));
        let img2 = image::RgbImage::from_fn(5, 5, |_, _| image::Rgb([100u8, 150u8, 200u8]));

        let correlation = calculate_correlation(&img1, &img2, 0.5);
        assert_eq!(
            correlation, 0.0,
            "Size mismatch should return 0.0 correlation"
        );
    }
}
